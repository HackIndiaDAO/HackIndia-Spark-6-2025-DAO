...
def rebuild_index(email, file_id):
    ...
    try:
        storage.delete_file(bucket_id=APPWRITE_BUCKET_ID, file_id=split_email)
    except:
        pass
    input_file = InputFile.from_path(temp_index_path)
    storage.create_file(bucket_id=APPWRITE_BUCKET_ID, file_id=split_email, file=input_file)
...
def upload_file_handler(request):
    ...
    try:
        ...
        result = storage.create_file(bucket_id=APPWRITE_BUCKET_ID, file_id=user_file_id, file=input_file)
        file_id = result['$id']
        ...
        return jsonify({"message": f"{filename} uploaded and indexed.", "viewer_url": preview_url})
...
def image_query_handler(request):
